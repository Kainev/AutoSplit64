...
import { Split, useRouteStore } from "../store";

const splitTypes = ["Star", "Bowser", "LBLJ", "Mips", "Custom"];

export const SplitTypeSelect = ({ split }: { split: Split }) => {
  ...
  return (
    <Field className="w-44">
      ...
      <Listbox
        ...
        onChange={(e) => updateSplit(split.id, { split_type: e })}
      >
        <ListboxButton
        ...
        >
          {splitTypes.map((splitType) => (
            ...
            >
              ...
              <div className="text-sm/6 text-white">{splitType}</div>
            </ListboxOption>
          ))}
        </ListboxOptions>
      </Listbox>
